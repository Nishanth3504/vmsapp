...
@Component({
  selector: 'app-legalnoticelist',
  templateUrl: './legalnoticelist.page.html',
  styleUrls: ['./legalnoticelist.page.scss'],
})
export class LegalnoticelistPage implements OnInit {
  ...
  ionViewDidEnter(){
    this.pageLength = 0;
    this.legalNoticeList = [];
    this.isInfiniteScrollDisabled = false;
    this.getLegalNoticesList('');
    this.filteredNoticeList =[];
  }

  
  getLegalNoticesList( searchText : any, event?: any) {
    ...
    this.moduleService.getLegalNoticesList(body).subscribe((res: any) => {
      console.log(res);
      ...
      if (res.statusCode == 200) {
        ...
        if (this.pageLength === 0) {
          this.legalNoticeList = res.data;
        } else {
          this.legalNoticeList = [...this.legalNoticeList, ...res.data];
        }
        
        this.filteredNoticeList = [...this.legalNoticeList]
  ...
  SearchNotices(event: any){
    ...
    this.SearchReportsTimeout = setTimeout(() => {
      ...
      this.getLegalNoticesList(searchTerm);
    }, 3000);
  }
  ...
  loadMoreNotices(event: any) {
    if (!this.isInfiniteScrollDisabled && !this.isSearching) {
      this.pageLength++;
      this.getLegalNoticesList(this.search_keyword, event);
    } else {
      event.target.complete();
    }
  }
