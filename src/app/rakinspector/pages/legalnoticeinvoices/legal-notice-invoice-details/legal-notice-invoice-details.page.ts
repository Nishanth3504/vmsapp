...
@Component({
  selector: 'app-legal-notice-invoice-details',
  templateUrl: './legal-notice-invoice-details.page.html',
  styleUrls: ['./legal-notice-invoice-details.page.scss'],
})
export class LegalNoticeInvoiceDetailsPage implements OnInit {
  ...
  agencyDetails: any;
   ...
   getnoticeInvoiceDetail() {
     this.moduleService.viewLegalNoticeInvoice({
       legal_invoice_id: this.routerData
     }).subscribe({
       next: (res: any) => {
         if (res.statusCode === 200) {
           console.log("view legal invoice notice", res);
           ...
           this.agencyDetails = res.lniAngencyData;
   ...
   submit() {
     ...
     this.geolocation.getCurrentPosition(options).then((position) => {
       ...
       this.moduleService.updateLegalNoticeInvoiceStatus(payload)
         .pipe(
           finalize(() => {
         ...
         .subscribe({
           next: (resp: any) => {
             if (resp.statusCode === 200) {
               ...
               this.router.navigate(['/legal-notice-invoice-list'], { replaceUrl: true });
             } else {
